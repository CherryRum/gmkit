...
- **ZUC** - 祖冲之序列密码算法（ZUC-128、标准 EEA3 消息加密、EIA3 MAC）
...
| ZUC | ZUC-128 密钥流、标准 EEA3 消息加密、EIA3 MAC；保留旧密钥流入口 | 不支持 ZUC-256；加密不自带完整性保护 |
| SM9 | 不支持 | Java 侧 `gmkit-sm9` 通过 JNI/GmSSL 提供，TS 侧无实现 |

### 国际标准算法

- **SHA** - SHA-1、SHA-256、SHA-384、SHA-512 系列哈希算法（当前不提供 SHA-224）
