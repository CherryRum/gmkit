...
assert.notEqual(sm3Hmac('merchant-demo-key', message), sm3Hmac('merchant-demo-key', tampered));
...
assert.notEqual(hmacSha256('merchant-demo-key', message), hmacSha256('merchant-demo-key', tampered));
...
assert.equal(sm4.decrypt(encrypted, { aad: 'tenant=demo;schema=1' }), message);
