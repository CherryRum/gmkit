...
::: tip 先确定载荷格式
完整流程、字段落库和篡改失败见 [Java SM2 + SM4 使用手册](/manual/java/hybrid.html)。本页只解释两个公开类型及其成员，不把对象的 `toString()` 当作协议格式。0.10.1 的默认配置是 `SM2-C1C3C2 + SM4-GCM + 12 字节随机 nonce + 16 字节 tag`。
:::
