...
| 项目 | 说明 |
|:--|:--|
| 默认用户 ID 变更 | 推荐使用空字符串 `''` 作为默认用户标识 |
| 密文模式明确 | 明确推荐使用 C1C3C2 模式作为标准密文格式 |
| 公钥格式规范 | 明确推荐使用非压缩格式（04 前缀）作为标准公钥格式 |
| 安全性增强 | 增强了对密钥长度和参数验证的安全建议 |
| 椭圆曲线参数 | 继续使用 GM/T 0003-2012 定义的曲线参数，无变化 |
...
| 项目 | 说明 |
|:--|:--|
| 内存优化 | 预分配输入缓冲区，避免重复分配 |
| 零值检测 | 在复制过程中同时检测是否有非零字节 |
| 错误处理 | 及时检测并报告无效的点乘结果 |
...
| 项目 | 说明 |
|:--|:--|
| 默认值保持不变 | DEFAULT_USER_ID 仍为 `'1234567812345678'` |
| 支持两种密文模式 | C1C3C2 和 C1C2C3 |
| 支持两种公钥格式 | 压缩和非压缩 |
| 自动格式检测 | 解密和验签时自动识别格式 |
| 显式配置优先 | 用户可以通过参数明确指定格式 |
...
| 项目 | 说明 |
|:--|:--|
| 空字符串用户 ID | 减少 Z 值计算的输入长度；可能带来轻微性能提升（以实测为准） |
| C1C3C2 密文模式 | 对性能无影响；主要是顺序差异，不影响计算量 |
| 非压缩公钥格式 | 无需椭圆曲线点解压计算；签名验签时可能略优（以实测为准）；存储和传输增加约 49% 开销 |
...
| v0.9.2 | 文档与示例对齐，补充标准与互操作说明 |

- **当前版本** (v0.9.2): 文档同步与规范修订，保持向后兼容
